...
CREATE INDEX IF NOT EXISTS idx_anomalies_org_unreviewed 
...
CREATE INDEX IF NOT EXISTS idx_ml_models_org_active 
...
CREATE TABLE IF NOT EXISTS public.access_recommendations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid REFERENCES public.organizations(id),
  user_id uuid REFERENCES auth.users(id),
...
CREATE INDEX IF NOT EXISTS idx_recommendations_user_pending 
...
CREATE TABLE IF NOT EXISTS public.risk_scores_history (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid REFERENCES public.organizations(id),
...
CREATE INDEX IF NOT EXISTS idx_risk_history_entity 
  ON public.risk_scores_history(entity_type, entity_id, calculated_at DESC);

CREATE INDEX IF NOT EXISTS idx_risk_history_org_level 
