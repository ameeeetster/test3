-- IMMEDIATE FIX: Run this in Supabase SQL Editor to fix orgs RLS
-- This enables RLS and creates policies to allow authenticated users to create orgs
...
DROP POLICY IF EXISTS "orgs_authenticated_update" ON public.orgs;
DROP POLICY IF EXISTS "orgs_select_org" ON public.orgs;
DROP POLICY IF EXISTS "orgs_modify_admin" ON public.orgs;

-- Allow authenticated users to see orgs they belong to (or all for bootstrap)
